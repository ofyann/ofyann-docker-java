...
├── .github/workflows/
│   └── docker-build.yml       # CI：每周一 UTC 00:00 + 手动触发（push 不构建）；matrix 同时构建 4 版本 × 2 变体 × 2 架构
...
2. `build-and-push` job：用 buildx 构建，`target` 为 `full`/`minimal`（单 Dockerfile）。`platforms: linux/amd64,linux/arm64`、`push: true`。先 `docker manifest inspect` 判断具体版本标签是否存在，存在则跳过（`force_build` 可强制）。双标签：`ofyann/java:<distro>-<major>[-minimal]` + `ofyann/java:<distro>-<full_version>[-minimal]`。
3. 测试仅跑 amd64（arm64 依赖 QEMU 仿真，较慢，只验证构建成功）。
4. `report` job：写 `GITHUB_STEP_SUMMARY`。

**触发方式**：仅 `schedule`（每周一 UTC 00:00）与 `workflow_dispatch`（手动，可 `force_build`）。**push 到 main 不触发构建**——避免每次推送消耗多架构 QEMU 构建额度。代码改动后需手动触发或在下个周一自动构建。

**所需 Secrets**：`DOCKERHUB_USERNAME`、`DOCKERHUB_TOKEN`。

> **成本控制**：每周一次全量多架构构建（4 版本 × 2 变体 × 2 架构，arm64 走 QEMU 较慢）。不含 arm/v7（QEMU 仿真下 java interpreter 解析不稳定）。免费额度 2000min/月下，若仍紧张可考虑改用 `eclipse-temurin` 官方 arm 镜像避免 QEMU。
...
**架构不进标签**。通过多架构 manifest list 实现，`docker pull` 自动选择匹配宿主架构（amd64/arm64）。如需调试显式拉取某架构，用 `docker pull --platform linux/arm64 ofyann/java:temurin-17`。
...
> - **多架构支持**：Dockerfile 按 `TARGETARCH` 自取架构直链+校验和；CI 全量构建 `linux/amd64,linux/arm64`（不含 arm/v7）。
...
1. **arm64 架构测试缺失**：CI 仅跑 amd64 运行时测试，arm64 仅验证构建成功。
...
4. **arm/v7 支持**：已从 CI 移除（QEMU 仿真下 java interpreter 解析不稳定）。Dockerfile 保留 `arm→arm` 映射分支供本地 buildx 扩展。如需恢复，需解决 QEMU armhf binfmt 问题或用 native ARM runner。
